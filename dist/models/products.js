"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
exports.ProductsModels = void 0;
var express_1 = __importDefault(require("express"));
var dotenv_1 = __importDefault(require("dotenv"));
var _a = require('pg'), Client = _a.Client, Pool = _a.Pool;
var database_1 = __importDefault(require("../database"));
dotenv_1["default"].config();
var router = express_1["default"].Router();
var userDataObj = {};
var itemDataObj = {};
var updateResponse = {};
var deletedUserObj;
// export type Items {
//     id: Number;
//     item_name: String;
//     item_description: String;
// }
var ProductsModels = /** @class */ (function () {
    function ProductsModels() {
    }
    ProductsModels.prototype.getAllProducts = function () {
        return __awaiter(this, void 0, void 0, function () {
            var resData, err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        // @ts-ignore
                        userDataObj = {};
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1["default"].connect()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, database_1["default"].query('SELECT * FROM products')];
                    case 3:
                        resData = _a.sent();
                        userDataObj = resData.rows;
                        if (resData.rowCount == 0) {
                            return [2 /*return*/, {
                                    "status": "Failure",
                                    "message": "Produts not found!!"
                                }];
                        }
                        else {
                            return [2 /*return*/, {
                                    "data": userDataObj
                                }];
                        }
                        return [3 /*break*/, 6];
                    case 4:
                        err_1 = _a.sent();
                        console.error(err_1);
                        return [3 /*break*/, 6];
                    case 5: return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    ProductsModels.prototype.getProductByName = function (product_name) {
        return __awaiter(this, void 0, void 0, function () {
            var res, err_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        // @ts-ignore
                        userDataObj = {};
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1["default"].connect()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, database_1["default"].query("SELECT * FROM products WHERE name ='" + product_name + "'")];
                    case 3:
                        res = _a.sent();
                        userDataObj = res.rows;
                        if (res.rowCount == 0) {
                            return [2 /*return*/, {
                                    "status": "Failure",
                                    "message": "Product was not found!!"
                                }];
                        }
                        else {
                            return [2 /*return*/, {
                                    "data": userDataObj
                                }];
                        }
                        return [3 /*break*/, 6];
                    case 4:
                        err_2 = _a.sent();
                        console.error(err_2);
                        return [3 /*break*/, 6];
                    case 5: return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    ProductsModels.prototype.addNewProducts = function (productName, productPrice) {
        return __awaiter(this, void 0, void 0, function () {
            var newItems, err_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        //@ts-ignore
                        updateResponse = {};
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1["default"].connect()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, database_1["default"].query("INSERT INTO products (name,price) VALUES ('" + productName + "','" + productPrice + "')")];
                    case 3:
                        newItems = _a.sent();
                        // const newItems = await client.query("INSERT INTO store_items (item_name,item_description) VALUES (" + item_Name + "," + item_Description + ")");
                        updateResponse = newItems.rows;
                        console.log(updateResponse);
                        if (newItems.rowCount == 0) {
                            return [2 /*return*/, {
                                    "status": "Failure",
                                    "message": "Items could not be inserted into the database"
                                }];
                        }
                        else {
                            return [2 /*return*/, {
                                    "status": "Successful",
                                    "message": "Record successfully added into the database",
                                    "data": newItems.rows
                                }];
                        }
                        return [3 /*break*/, 6];
                    case 4:
                        err_3 = _a.sent();
                        console.error(err_3);
                        return [3 /*break*/, 6];
                    case 5: return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    ProductsModels.prototype.updateProductInfo = function (productId, productPrice) {
        return __awaiter(this, void 0, void 0, function () {
            var updatedData, err_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        return [4 /*yield*/, database_1["default"].connect()];
                    case 1:
                        _a.sent();
                        return [4 /*yield*/, database_1["default"].query("UPDATE products SET price = '" + productPrice + "' WHERE id = '" + productId + "'")];
                    case 2:
                        updatedData = _a.sent();
                        if (updatedData.rowCount == 0) {
                            return [2 /*return*/, {
                                    "status": "Failed",
                                    "message": "Failed to update record"
                                }];
                        }
                        else {
                            return [2 /*return*/, {
                                    "status": "Successful",
                                    "message": "Record updated successfully"
                                }];
                        }
                        return [3 /*break*/, 4];
                    case 3:
                        err_4 = _a.sent();
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    ProductsModels.prototype.deleteProduct = function (productId) {
        return __awaiter(this, void 0, void 0, function () {
            var deletedProduct, err_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, 4, 5]);
                        return [4 /*yield*/, database_1["default"].connect()];
                    case 1:
                        _a.sent();
                        return [4 /*yield*/, database_1["default"].query("DELETE FROM products WHERE id ='" + productId + "'")];
                    case 2:
                        deletedProduct = _a.sent();
                        // deletedUserObj = deletedProduct;
                        // console.log(deletedUserObj);
                        if (deletedProduct.rowCount == 0) {
                            return [2 /*return*/, {
                                    "status": "Failure",
                                    "message": "Record failed to be deleted from the database"
                                }];
                        }
                        else {
                            return [2 /*return*/, {
                                    "status": "Successful",
                                    "message": "Record successfully deleted from the database"
                                }];
                        }
                        return [3 /*break*/, 5];
                    case 3:
                        err_5 = _a.sent();
                        console.error(err_5);
                        throw err_5;
                    case 4: return [7 /*endfinally*/];
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    return ProductsModels;
}());
exports.ProductsModels = ProductsModels;
